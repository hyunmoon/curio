...
table. Consequently, one bad row or an aggregate deal larger than the entire
active cap does not permanently block later valid deals. A deal that does not
fit the currently available capacity remains waiting and can be reconsidered
after slots are returned.
...
When either setting is positive, Curio performs fresh MK20 and sector pressure
checks at the start of every pass. Pressure, or an error while checking it,
stops only new waiting releases for that pass. Existing pipeline work keeps
progressing and the next pass checks again. When both settings are zero, this
additional fresh pressure check is disabled; the existing cached API pressure
behavior and its cache lifetime are unchanged.
...
on the production YugabyteDB version. Before treating the change as production
ready, run the opt-in integration tests against an isolated disposable
YugabyteDB instance and record the server version and effective isolation
level. At minimum, validate:
