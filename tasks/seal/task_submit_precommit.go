...
func calculatePrecommitExpiration(
	...
	if len(pieces) == 0 {
		...
		for _, piece := range pieces {
			for _, start := range []sql.NullInt64{piece.F05DealStartEpoch, piece.DirectDealStartEpoch} {
		...
		expiration = max(hardF05End, min(requestedDirectEnd, protocolMaxExpiration))
	}
