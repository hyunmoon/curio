...
func TestWorkerUnavailableSQLOrdinaryProbeSpendsBudget(t *testing.T) {
	...
	prepareRetryFixtureAttempt(t, ctx, db, 101, 1, "failed-backend")
	...
	require.Equal(t, 8, h.recordCompletion(1, nil, time.Now(), false, cause, false, "failed-backend").Retries)
	...
	require.NoError(t, err)
	prepareRetryFixtureAttempt(t, ctx, db, 101, 1, "ordinary-probe")
	ordinary := errors.New("invalid proof")
	require.Equal(t, 2*time.Minute, gate.Result(gate.Epoch(), ordinary))
	retry := h.recordCompletion(1, nil, time.Now(), false, ordinary, false, "ordinary-probe")
...
func TestWorkerUnavailableSQLPreservesBudgetAndAttemptIdentity(t *testing.T) {
	...
	require.NoError(t, err)
	prepareRetryFixtureAttempt(t, ctx, db, 101, 1, "first-attempt")
	...
	retry := h.recordCompletion(1, nil, time.Now(), false, cause, false, "first-attempt")
	require.NotNil(t, retry)
	require.Equal(t, 9, retry.Retries)
	var failures int
	require.NoError(t, db.QueryRow(ctx, `SELECT count(*) FROM harmony_task_history WHERE NOT result`).Scan(&failures))
	...
	require.NoError(t, err)
	prepareRetryFixtureAttempt(t, ctx, db, 102, 1, "new-attempt")
	require.Nil(t, h.recordCompletion(1, nil, time.Now(), false, cause, false, "first-attempt"))
	var owner int
	var token string
	...
	h.recordCompletion(1, nil, time.Now(), false, errors.New("invalid sector proof"), false, "new-attempt")
	var remaining int
