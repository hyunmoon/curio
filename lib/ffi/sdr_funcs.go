...
func (l *storageProvider) AcquireSector(ctx context.Context, taskID *harmonytask.TaskID, sector storiface.SectorRef, existing, allocate storiface.SectorFileType, sealing storiface.PathType) (fspaths, ids storiface.SectorPaths, release func(dontDeclare ...storiface.SectorFileType), err error) {
	...
	accessRelease, err := sdrscratch.AccessPathsContext(ctx, sectorPaths.Cache, sectorPaths.Key)
	if err != nil {
		releaseStorage()
		return storiface.SectorPaths{}, storiface.SectorPaths{}, nil, err
	}
...
func (sb *SealCalls) generateSDRBody(ctx context.Context, taskID harmonytask.TaskID, into storiface.SectorFileType, sector storiface.SectorRef, ticket abi.SealRandomness, commDcid cid.Cid, generate func(abi.RegisteredSealProof, string, [32]byte) error, beforeCleanup func(string) error, scratchOptions sdrscratch.Options, ticketEpoch ...abi.ChainEpoch) (retErr error) {
	...
	want, err := newSDRReceipt(into, sector, ticket, commDcid, ticketEpoch)
	if err != nil {
		return err
	}
	...
	r, err := readSDRReceipt(intoPath)
	if err != nil {
		return err
	}
	if r != nil {
		if !r.matches(want) {
			return xerrors.New("published SDR inputs differ from this attempt")
		}
		published = true
		reservation.UsePublished(intoPath)
		return sb.ensureOneCopy(ctx, sector.ID, pathIDs, into)
	}
	// A previously credited published cache must not disappear and then fund a
	// fresh generation. A new claim must reserve the full scratch budget.
	if reservation.Published() {
		return xerrors.New("reserved SDR output disappeared; reclaim storage before retry")
	}
	...
	if onlyLastLayer {
		...
		if err := writeSDRReceipt(filepath.Join(intoTemp, lastLayer), want); err != nil {
			return xerrors.Errorf("recording SDR completion: %w", err)
		}
		...
		if err := writeSDRReceipt(intoTemp, want); err != nil {
			return xerrors.Errorf("recording SDR completion: %w", err)
		}
